...
namespace EditorScene {
    ...
    public:
        ...
        DirectionalLightElement(const ElementRef& parent, std::string name, glm::vec3 position, std::shared_ptr<DirectionalLight> light, std::shared_ptr<EmissiveEntityRenderer::Entity> light_arrow) :
            SceneElement(parent, std::move(name)), position(position), light(std::move(light)), light_arrow(std::move(light_arrow)) {}
